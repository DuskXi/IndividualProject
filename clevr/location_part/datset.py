...
class ClevrBoxPositionDatasetV3(Dataset):
    def __init__(self, scene_path, num_objects=5):
        with open(scene_path, 'r') as f:
            data = json.load(f)
        self.data = data["scenes"]
            ...
            for i, line in enumerate(adj_list):
                for v in line:
                    try:
                        ...
                        print("")
            relationships[key] = adj_matrix
        return torch.tensor(np.array(boxes), dtype=torch.float32), torch.tensor(np.array([relationships[k] for k in ["left", "right", "front", "behind"]]), dtype=torch.float32)
