...
def display_image(image, y, label_map, dpi=100, draw_width=1080, title="", index=False, headless=False):
    ...
    d = []
    for i, (box, label) in enumerate(zip(y["boxes"], y["labels"])):
        d.append({
            "box": box / image.shape[1] * draw_width,
            "text": (str(i) if index else "") + label_map[label]
        })
    # opencv rgb -> bgr
