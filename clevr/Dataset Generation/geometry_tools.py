...
def simple_rasterization(objects, mvp_matrices, width, height):
    # projection to ndc
    ndc_vertices = {}
    ...
    frameBuffer = np.zeros((width, height, 1), dtype=np.int32)
    frameBuffer.fill(-1)
    for i, obj in enumerate(objects):
        ...
        for face in mesh.polygons:
            ...
            if len(vertex_indices) == 3:
                triangles.append([np.array(ndc_vertices[obj.name][i]) for i in vertex_indices] + [i])
            elif len(vertex_indices) == 4:
                quads = [np.array(ndc_vertices[obj.name][i]) for i in vertex_indices]
    ...
    for i, obj in enumerate(objects):
        ...
        count_other = (screen_box[1] - screen_box[0]) * (screen_box[3] - screen_box[2]) - count_obj
